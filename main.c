...
void * clerk(void* clerk_id_ptr)
{
    ...
    while(1)
    {
        ...
        cur_simulation_secs = getCurrentSimulationTime();
        printf("%d -->>> A clerk finishes serving a customer: end time %0.2f, the customer ID %d, the clerk ID %d.", (int)(cur_simulation_secs * 10), cur_simulation_secs, cust_id, clerk_id + 1);
